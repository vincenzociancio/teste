...
{
    class Pre_Processos_Emails
    {
        #region Campos
        int _PK_ID;
        string _FK_preProcesso;
        string _Email_ID;
        string _Email_De;
        string _Email_Para;
        string _Email_CC;
        ...
        {
            get { return _FK_preProcesso; }
            set { _FK_preProcesso = value; }
        }
        ...
        public bool InsertProcessos_Emails()
        {
            ...
            try
            {
                SqlCommand comselect = new SqlCommand("select FK_preProcesso, Email_ID, Email_Assunto from Pre_Processos_Emails where FK_preProcesso='" + FK_Processo + "' and Email_ID='" + Email_ID + "' and Email_Assunto='" + Email_Assunto + "'", db.conexao);
                ...
                if (!dr.HasRows)
                {
                    ...
                    SqlCommand comm = new SqlCommand("insert into Pre_Processos_Emails (FK_preProcesso, Email_ID, Email_De, Email_Para, Email_CC, Email_CCO, Email_Assunto, Email_BodyHTML, Email_BodyHTMLOriginal, Email_BodyTXT, Email_Importancia, DT_Email, DT_Cadastro, FK_Usuario) " +
                                                     "values(@FK_preProcesso, @Email_ID, @Email_De, @Email_Para, @Email_CC, @Email_CCO, @Email_Assunto, @Email_BodyHTML, @Email_BodyHTMLOriginal, @Email_BodyTXT, @Email_Importancia, @DT_Email, @DT_Cadastro, @FK_Usuario)", db.conexao);

                    comm.Parameters.Add("@FK_preProcesso", SqlDbType.VarChar).Value = FK_Processo;
        ...
        /// <returns></returns>
        public DataTable SelectViculados(int id_pre_processo)
        {
            try
            {
                ...
                var sql = "select top 10 Email_De as 'De', Email_Para as 'Para', Email_Assunto as 'Assunto', FK_Usuario as 'Usuário', DT_Cadastro as 'Vinculação' from Pre_Processos_Emails where FK_preProcesso ='" + id_pre_processo + "' order by PK_ID desc ";
        ...
        public int GetIDProcessosEmails(string Usuario)
        {
            ...
            try
            {
                SqlCommand comm = new SqlCommand("select Max(PK_ID) as ID from Pre_Processos_Emails where FK_Usuario ='" + Usuario + "'", db.conexao);
        ...
        public void UpdateBody()
        {
            ...
            try
            {
                SqlCommand comm = new SqlCommand("update Pre_Processos_Emails set Email_BodyHTML = @Email_BodyHTML " +
