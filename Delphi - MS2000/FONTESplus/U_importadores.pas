...
begin
  ...
  vnome_planilha := 'Relatório_de_Importadores';
  ...
  // todos
  Excel.Workbooks[1].Worksheets['Plan1'].Name := 'Relatório_de_Importadores';
  Excel.WorkSheets['Relatório_de_Importadores'].Select;
  Excel.WorkSheets['Relatório_de_Importadores'].Shapes.AddPicture ('c:\ms2000\logoms.jpg', True, True, 1, 0, 145, 65);
  ...
  Excel.Cells[2,1] := vespacos + 'Relatório de Importadores';
  ...
  Celtitulo(linha, 3,'Endereço', clNavy, ACenter, clWhite, 10, 36);
  ...
  if MessageDlg('Deseja Gerar Somente os Importadores Ativos? ', mtConfirmation,[mbYes, mbNo], 0) = mrYes
   then begin
     ...
     qrImportadores.sql.Add('case when[Acesso_WEB_MS] = 1 then ''Sim'' else ''Não'' end  as Acesso_WEB_MS,');
     qrImportadores.sql.Add('case when[Acesso_WEB_CLI]  = 1 then ''Sim'' else ''Não'' end  as Acesso_WEB_CLI,');
     qrImportadores.sql.Add('case when[Acesso_WEB_REC] = 1 then ''Sim'' else ''Não'' end  as Acesso_WEB_REC,');
  ...
  if SaveDialog4.Execute
   then begin
     if FileExists(SaveDialog4.FileName)
      then if MessageDlg(Format('Você deseja salvar esse arquivo?', [SaveDialog4.FileName]),mtConfirmation, mbYesNoCancel, 0) <> idYes
            then Exit;
...
begin
  ...
  if q_Proc.RecordCount <> 0
   then MessageDlg('Existem Processos com este Importador. Não pode ser excluído!', mtInformation,[mbOk], 0)
   else if MessageDlg('Confirma exclusão deste Importador?', mtConfirmation,[mbYes, mbNo], 0) = mrYes
         then begin
           while not t_Resp.Eof do
             t_Resp.Delete;
             t_Importadores.Delete;
             MessageDlg('Importador excluído!', mtInformation,[mbOk], 0);
         end;
  q_Proc.Close;
  btnNovo.SetFocus;
end;
...
begin
  if edtRazao.Text = ''
   then begin
     MessageDlg('Digite a Razão Social do Importador!', mtInformation,[mbOk], 0);
     edtRazao.SetFocus;
     Exit;
   end;
  ...
  if qr_VerificaImportadores.RecordCount <> 0
   then begin
     if vIMP <> edtCnpj.Text
      then begin
        MessageDlg('Importador já Cadastrado!', mtWarning,[mbOk], 0);
        edtCnpj.SetFocus;
  ...
  if op = 1
   then begin
     ...
     MessageDlg('Alteração do Importador feita com sucesso!', mtInformation,[mbOk], 0);
   end;
...
begin
  ...
  if MessageDlg('Esse arquivo será associado ao contrato '+qContratosnumero_contrato.AsString+', confirma o upload ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
   then begin
      with qContrato do
      begin
        Close;
          ...
          then begin
            try
              ...
              if qImportadoresAgrupadosContratos.RecordCount > 1
               then begin
                 if MessageDlg('Esse mesmo contrato está associado aos seguintes importadores: '+#13#10+#13#10+Importadores+#13#10+#13#10+'Deseja fazer o upload desse arquivo para todos ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
                  then begin
                    qImportadoresAgrupadosContratos.First;
      ...
      if vOper = 'I'
       then begin
         ...
         if qImportadoresAgrupados.RecordCount > 1
          then begin
            if MessageDlg('Esse importador está associado a um grupo com os seguintes importadores vinculados: '+#13#10+#13#10+Importadores+#13#10+#13#10+'Deseja cadastrar essa proposta para todos ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
             then begin
               qImportadoresAgrupados.First;
...
begin
  ...
  if (Trim(txtVigInicial.Text) <> '')
   then begin
     ...
     on EConvertError do
       begin
         ShowMessage ('Por favor, informe uma data de vigência inicial válida.');
  ...
  if (Trim(txtVigFinal.Text) <> '')
   then begin
     ...
     on EConvertError do
       begin
         ShowMessage ('Por favor, informe uma data de vigência final válida.');
  ...
  if (Trim(txtAprovacao.Text) <> '')
   then begin
     try
      ...
      on EConvertError do
       begin
         ShowMessage ('Por favor, informe a data de aprovação válida.');
