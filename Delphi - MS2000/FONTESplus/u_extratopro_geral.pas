...
{  If MessageDlg('somente ADMISSÃO?', mtConfirmation,[mbYes, mbNo], 0) = mrNo Then   vsomenteADM := false
 ...
 vsomenteADM := false;
  ...
  If MessageDlg('Verifica apenas relação?', mtConfirmation,[mbYes, mbNo], 0) = mrNo Then vrelacao := false
  else vrelacao := true;
  }
...
{           Excel.ActiveWorkBook.VBProject.VBComponents.Item('plan1').Codemodule.CreateEventProc ('activate' , 'worksheet');
           ...
           While Not qrExcelProc.Eof Do Begin
              ...
              if fileexists(v_path_server + 'Docs\' + v_dirDocs + qrExcelProcArquivo.AsString +'.jpg') then begin
                 ...
                 showmessage('Arquivo não encontrado - Processo:'+v_procCodigo+' - Doc:'+v_procNumero+' - '+v_procDesc);
              end;
          ...
          If cb_cuu.Checked Then Begin
{
             ...
             Else
                Planilha := 'CUSTOS UNITÁRIOS - ANALÍTICO (PREVISÃO)';
...
//           planilha := v_procNumero + ' - ' + v_procDesc;
           ...
           Begin
              ...
              begin
                 ...
                 if c > 1 then c:=c+1;
                 //showmessage('Arquivo não encontrado - Processo:'+v_procCodigo+' - Doc:'+v_procNumero+' - '+v_procDesc);
                 Excel.Cells[2,c] := 'Arquivo não encontrado - Processo:'+v_procCodigo+' - Doc:'+v_procNumero+' - '+v_procDesc;
              end;
...
//  image1.Picture.SaveToFile('c:\ms2000\logoms.jpg');
  ...
  osprocessos.first;
   ...
   Excel.Cells[2,3] := 'EXTRATO DE PROCESSOS - V 1.0 - '+ FormatDateTime('dd/mm/yyyy "às" hh:nn', Now());
...
//   Excel.Cells[4,1] := Planilha;
   ...
   Celtitulo(linha2, 7,'Tipo Declaração', clNavy, ACenter, clNavy, 8, 35);
...
{  Excel.ActiveWorkBook.VBProject.VBComponents.Item('plan1').Codemodule.CreateEventProc ('activate' , 'worksheet');
  ...
  Excel.ActiveWorkBook.VBProject.VBComponents.Item('plan1').CodeModule.VBE.MainWindow.Visible := False;
   ...
   Excel.Cells[2,1] := 'EXTRATO DE PROCESSOS - V 1.0 - '+ FormatDateTime('dd/mm/yyyy "às" hh:nn', Now());
   Excel.Cells[3,1] := 'RELAÇÃO DE PROCESSOS: TRANSOCEAN BRASIL LTDA' ;
   Excel.Cells[1,1].Font.Size := 12;
