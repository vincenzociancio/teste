...
type
  Tqr_extrato_financ = class(TQuickRep)
    DetailBand1: TQRBand;
    titulo: TQRBand;
...
var
  qr_extrato_financ: Tqr_extrato_financ;
...
procedure Tqr_extrato_financ.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if f_contascorrentes.q_extratovalor.asfloat >= 0
   then begin
...
procedure Tqr_extrato_financ.titulocreditosAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  qrl_subtotcred.caption  := '0';
end;

procedure Tqr_extrato_financ.titulodebitosAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  qrl_subtotdeb.caption  := '0';
end;

procedure Tqr_extrato_financ.subcreditosAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  qrl_subtotcred.caption  := floattostrf(strtofloat(qrl_subtotcred.caption)+f_contascorrentes.q_creditos_financvalor.asfloat,fffixed,15,2);
end;

procedure Tqr_extrato_financ.subdebitosAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  qrl_subtotdeb.caption  := floattostrf(strtofloat(qrl_subtotdeb.caption)+f_contascorrentes.q_debitos_financvalor_contabilizado.asfloat,fffixed,15,2);
end;

procedure Tqr_extrato_financ.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  ...
  qr_extrato_financ.qrl_saldo.caption     := floattostrf(f_contascorrentes.q_santeriorsaldoanterior.asfloat,fffixed,15,2);
end;
