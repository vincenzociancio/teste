...
//        Application.CreateForm(TQR_TR, QR_TR);
     ...
     If MessageDlg('Imprimir data de Vencimento no TR?',mtConfirmation,[mbYes, mbNo], 0) = mrYes Then begin
        If MessageDlg('Imprimir Somente data de Vigência Final no TR?',mtConfirmation,[mbYes, mbNo], 0) = mrYes Then
...
begin
     If tbProcessos.State In [dsEdit] Then Begin
        MessageDlg('O TR está em edição!', mtInformation,[mbOk], 0);
        CanClose := False;
     End
     Else
        CanClose := True;
...
begin
     {Leandro Serra - 31/08/2011
        Solicitação da Sra Odilma
        O usuário só poderá salvar se o documento Nº da TR estiver escaneado}
      ...
      begin
         ...
         begin
            MessageDlg(v_usuario+',  somente é permitido salvar o registro se houver o NÚMERO TR digitalizado.', mtWarning, [mbOk], 0);
            exit;
         end;
     end;
     {Fim 31/08/2011}
     ...
     If dbEdtDtVenc.Text = '  /  /    ' Then Begin
        MessageDlg(v_usuario+',  Atenção! "Vigência Final" está em branco!', mtWarning, [mbOk], 0);
...
//        Exit;
     ...
     If dbEdtDtEntrada.Text <> '  /  /    ' Then
        If StrToDate(dbEdtDtEntrada.Text) > StrToDate(dbEdtDtVenc.Text) Then Begin
           MessageDlg(v_usuario+',  "Vigência Final" é menor que a "Vigência Inicial"!', mtWarning, [mbOk], 0);
           dbEdtDtVenc.SetFocus;
           Exit;
        End;
     end;
     ...
     Else
        ...
        Except
           MessageDlg('Valor FOB inválido!', mtWarning, [mbOk], 0);
           edtValFOB.SetFocus;
           Exit;
        End;
     ...
     Else
        ...
        Except
           MessageDlg('Valor CIF inválido!', mtWarning, [mbOk], 0);
           edtValCIF.SetFocus;
           Exit;
        End;
     ...
     Else
        ...
        Except
           MessageDlg('Imposto de Importação inválido!', mtWarning, [mbOk], 0);
           edtImposto.SetFocus;
           Exit;
        End;
     ...
     Else
        ...
        Except
           MessageDlg('IPI inválido!', mtWarning, [mbOk], 0);
           edtIPI.SetFocus;
           Exit;
        End;
     ...
     Else
        ...
        Except
           MessageDlg('COFINS inválido!', mtWarning, [mbOk], 0);
           edtCOFINS.SetFocus;
           Exit;
        End;
     ...
     Else
        ...
        Except
           MessageDlg('Total dos Impostos inválido!', mtWarning, [mbOk], 0);
           edtTotal.SetFocus;
           Exit;
        End;
...
//                 (tbDocImportador.AsString   = tbProcessosImportador.AsString) And
     ...
     End;
         ...
         Showmessage(v_usuario+', código de arquivo inválido não foi possível incluir no controle de documentos.')
     end;
...
begin
  Application.MessageBox(PChar('Número TR sem mascara: '+tbProcessosNumero_TR.AsString ), 'Visualização Nº TR', MB_OK+MB_ICONINFORMATION);
end;
  ...
  if (Length(F_MSCOMEX.LRTrim(me_nossaref.Text)) = 8)
   then begin
     if F_MSCOMEX.IncluiFollow(ME_nossaref.Text, '952', '0', '', v_usuario, date, time())
      then begin
        Showmessage(v_usuario+', ATENÇÃO! Foi INCLUÍDO Follow up de Termo de Responsabilidade ORIGINAL arquivado pasta controle (MS) para o Processo: ' + pchar(ME_nossaref.Text)+'.');
