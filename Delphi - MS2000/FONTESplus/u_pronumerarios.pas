  ...
  if ( Not qrProcessos.IsEmpty )
   then begin
    ...
    // Fim Eduardo Souza - 26/03/2012 - Ticket : 1773
     ...
     //mm 08/11/2007
     Showmessage(v_usuario+',Atenção!! Numerários contabilizados pelo Cliente. Após a informação de Numerário Contabilizado pelo Cliente, NÃO será possível alterar. Somente o Financeiro poderá fazer esta alteração pelo Faturamento!');
...
begin
  ...
  if( qrySelectAnexo.IsEmpty ) Then Begin
    Application.MessageBox( 'Este processo não possui o anexo da ANÁLISE DO SISCOMEX. Para prosseguir você deverá anexar a imagem da ANALISE DO SISCOMEX na tela de DI/ DSI.', 'Aviso', MB_OK+MB_ICONWARNING );
    Abort;
  End;
  ...
  If Not( ValoresOK ) Then Begin
    Application.MessageBox('Email não pode ser enviado, pois existe numerário com valor previsto ZERADO!', 'Aviso', MB_OK+MB_ICONWARNING );
    Abort;
  End;
  {$ENDIF}
  // Fim Eduardo Souza - 26/03/2012 - Ticket : 1773
  ...
  {$ELSE}
    ...
    If ( qrySupervisores.IsEmpty ) Then Begin
      ...
      End
      Else Begin
        Application.MessageBox( 'O usuário não possui supervisor no cadastro.', 'Aviso', MB_OK+MB_ICONWARNING );
        Abort;
      End;
    End
    Else Begin
    ...
    Repeat
      ...
      Click := InputQuery( 'Autorização para envio', 'Entre com a senha do supervisor: '+ Supervisor, Senha );
  ...
  if MessageDlg('Gera Email em Inglês?', mtConfirmation,[mbYes, mbNo], 0) = mryes
   then emPortugues := 1;

  if MessageDlg('Gera Email apenas dos NÃO SOLICITADOS?', mtConfirmation,[mbYes, mbNo], 0) = mrYes
   then soNaoSoli := 1;
  ...
  memo_email.Lines.Add('ATENÇÃO: Após anexar os documentos necessários encaminhar:                      ');
  memo_email.Lines.Add('--->Para: '+tbContatosImpEmail_Numerario.AsString);
  memo_email.Lines.Add('--->Com Cópia Para: '+tbContatosImpCC_Email.AsString);
  memo_email.Lines.Add('________________________________________________________________________________');
  memo_email.Lines.Add('                                                                                ');
  memo_email.Lines.Add('                                                                                ');
  ...
  for L := Low(vetSOL) to High(vetSOL) do
   begin
     if emPortugues = 0
      then begin
        email_num.PostMessage.Subject := 'MS2000PLUS - Solicitação de Numerários - '+qrProcessosCliente.AsString+' - Processo: '+qrProcessosProcesso.AsString;
         ...
         //Fim----------------------------------------
           with memo_email.Lines do
            begin
              ...
              Add('                                                                                ');
              Add('                                                                                ');
              Add('Solicitamos numerários referentes ao processo: '+qrProcessosProcesso.AsString+', Ref.Cliente:'+ qrProcessosRefCli.AsString+', ');
           ...
           if qembarcacao <> ''
            then begin
              memo_email.Lines.Add('embarcação '+qembarcacao + ', conhecimento '+Trim(qrConhecimentoDescricao.AsString)+' - '+Trim(qrConhecimentoNumerodoccarga.AsString));
           ...
           if (not TEM_PARASOL) or (pos(qrNumerariosnumerario.asstring,vetsol[L,1])>0)
            then begin
              if qrNumerariosdeb_cli.asstring='SIM'
               then begin
                 if ( soNaoSoli = 1 ) and (qrNumerariosPrevisao_Solicitada.AsString = 'NÃO')
                  then begin
                    TEM_DD := true;
                    qrNumerarios.last;
                  end
        ...
        if TEM_DD
         then begin
           with memo_email.Lines do
            begin
              Add('                                                                                ');
              Add('Especificação de Numerários - Débitos Diretos_______ Valor Previsto             ');
              Add('                                                                                ');
            end;
        ...
        with memo_email.Lines do
         begin
            Add('                                                                                ');
            Add('                                                                                ');
            Add('Especificação de Numerários - Adiantamentos MS______ Valor Previsto             ');
        ...
        with memo_email.Lines do
         begin
           ...
           if valtotalDD > 0
            then begin
              Add(Copy('Total Débitos Diretos____________________________________',1,52)+' '+FloatToStrF(valTotalDD,fffixed,10,2));
              Add('                                                                                ');
            end;
       ...
       begin
         if TEM_PARASOL
          then begin
            ...
            if valtotalDDG > 0
             then begin
               Add(Copy('Total Débitos Diretos____________________________________',1,52)+' '+FloatToStrF(valTotalDDG,fffixed,10,2));
               Add('                                                                                ');
             end;
         ...
         Add('                                                                                ');
         Add('                                                                                ');
         Add('Observações: Os valores mencionados acima são previsões, sofrerão alterações    ');
         Add('conforme a variação cambial da moeda negociada.                                 ');
         Add('                                                                                ');
         Add('Favor efetuar depósito identificado.                                            ');
         Add('Os créditos efetuados em cheque estão sujeitos a compensação bancária.          ');
         Add('                                                                                ');
         Add('Dados para Depósito:');
         Add('                                                                                ');
         Add('Banco___________ '+qrCCDepositoBanco.AsString);
         Add('Agência_________ '+qrCCDepositoAgencia.AsString);
         Add('Conta Corrente__ '+qrCCDepositoConta_Corrente.AsString);
         Add('CNPJ____________ 30.875.884/0001-99');
         Add('Razão Social____ MS Logistica Aduaneira Ltda.');
         Add('                                                                                ');
         Add('Atenciosamente,                                                                 ');
         Add('                                                                                ');
         Add('                                                                                ');
...
begin
  ...
  if MessageDlg('Gera Solicitação em Inglês?', mtConfirmation,[mbYes, mbNo], 0) = mrNO
   then begin
     emPortugues := 1;
  ...
  if MessageDlg('Gera Solicitação apenas dos NÃO SOLICITADOS?', mtConfirmation,[mbYes, mbNo], 0) = mrYes
   then qrNumerarios.Filter := 'NUMERARIO <> ''ISS'' AND NUMERARIO <> ''CPMF'' AND NUMERARIO <> ''TAXE'' AND PREVISAO_SOLICITADA = ''NÃO'' '
  ...
  if MessageDlg('Confirma Solicitação desta Previsão?', mtConfirmation,[mbYes, mbNo], 0) = mrYes
   then begin
     qrNumerarios.First;
...
begin
  //mm 07062006
  {qmsg := #13#10#13#10+'As solicitações de numerários serão encaminhadas para o Setor Financeiro,'+#13#10;
  qmsg := qmsg+'após análise, o Setor Financeiro encaminhará a Solicitação ao Cliente se necessário'+#13#10;
  ...
  if F_MSCOMEX.t_parametroshost_smtp.asstring = ''
   then begin
   ...
   else begin // envia com authenticação
     ...
     try
       try
         ...
         qryEmail_Auto.ParamByName( 'Titulo' ).Value := 'Numerários';
         ...
         // Controle de Autorização de numerários
         {$IFDEF Testando}
        ...
        {$ELSE}
          // não faz nada
        {$ENDIF}
         // Follow Automático eduardo.souza 18/07/2011
         F_MSCOMEX.IncluiFollow( qrProcessosProcesso.AsString, '1021','1147', '', v_usuario, Date, Now );
         logusu('O','Tela de Numerários - Envio de Numerários autorizado por: '+ Supervisor );
         // fim 18/07/2011
  ...
  if deuerro
   then showmessage(v_usuario+', seu email não foi enviado!')
   else begin
...
begin
  ...
  logusu('P','Alterou o Numerário: '+tbNumerariosNumerario.AsString+', do Processo: '+tbNumerariosProcesso.AsString);
end;

procedure TF_proNumerarios.tbNumerariosBeforeDelete(DataSet: TDataSet);
begin
  logusu('E','Excluiu o Numerário: '+tbNumerariosNumerario.AsString+', do Processo: '+tbNumerariosProcesso.AsString);
end;
...
begin
  ...
  if (v_AlteraPRO = 1)
   then begin
     {Solicitação da Odilma:
      Permitir solicitação de numerários somente se houver DI enviada p/ Anãlise.
      Primeiro verificar se o processo tem DI.
      Leandro Serra - 18/08/2011}

     {Solicitação da Odilma: Alterado para permitir somente se tiver informado
      Setor Operacional - Enviou Declaração de Importação para análise no SISCOMEX
      ...
      then begin
        if (q_Processo_Tem_DINR_DECLARACAO_IMP.Value <> '') and not (q_Processo_Tem_DINR_DECLARACAO_IMP.IsNull)
         then if q_Informou_FollowDI_Enviada.RecordCount > 0
               ...
               else Showmessage(v_usuario+', ATENÇÃO! Informe o follow Setor Operacional - Enviou Declaração de Importação para análise no SISCOMEX, para solicitar os numerários.')
...
begin
  ...
  tbNumerariosUsuario_Previsao.AsString       := v_usuario;
  tbNumerariosPrevisao_Solicitada.AsString    := 'NÃO';
  tbNumerariosOk_Previsao_financeiro.asstring := 'NÃO';
     ...
     if Not(Obrigatorio)
      then begin
        if (tbNumerariosValor_Registrado.AsFloat > 0)
         then begin
           MessageDlg('Numerário não pode ser excluído! valor registrado maior que zero.', mtInformation,[mbOk], 0);
           dbGrdNumerarios.SetFocus;
           Exit;
         end;
        ...
        if (tbNumerarioscontabilizado_cliente.asinteger = 1)
         then begin
           MessageDlg('Numerário não pode ser excluído! Contabilizado pelo cliente.', mtInformation,[mbOk], 0);
           dbGrdNumerarios.SetFocus;
           Exit;
         end;
        ...
        if (tbNumerarioscontabilizado.asstring = 'SIM')
         then begin
           MessageDlg('Numerário não pode ser excluído! Contabilizado no Financeiro.', mtInformation,[mbOk], 0);
           dbGrdNumerarios.SetFocus;
           Exit;
         end;

        if MessageDlg('Confirma exclusão do Numerário?', mtConfirmation,[mbYes, mbNo], 0) = mrNo
         then Exit;
      ...
      else Showmessage(v_usuario+', ATENÇÃO! Esse numerário é obrigatório para esse processo e não pode ser excluído.');
   end
   else Showmessage(v_usuario+', ATENÇÃO! Acesso não permitido à processos sob Responsabilidade e/ou o processo está fechado e/ou o Cliente está Inativo.');
end;
  ...
  if (v_AlteraPRO = 1)
   then begin
     {Leandro Serra - 12/08/2011
      Verifica se o tipo de numerário for Frete ou Armazenagem deve-se informar o respectivo ao tipo de processo.
      No caso eu criei um tipo genérico FRET este não é contabilizado então o usuário não poderá gravá-lo, o mesmo
     ...
     if tbNumerariosNumerario.IsNull
      then begin
        ShowMessage('O Numerário está em branco!');
        dbLcbNumerario.SetFocus;
        Exit;
      end;
     ...
     if tbNumerariosData_Previsao.IsNull
      then begin
        ShowMessage('A Data de Previsão está em branco!');
     ...
     if tbNumerariosValor_Previsao.IsNull
      then begin
        ShowMessage('O Valor de Previsão está em branco!');
     ...
     if tbnumerarioscontabilizado_cliente.asinteger = 1
      then begin
        if ((tbNumerariosValor_registrado.IsNull) or (tbNumerariosValor_registrado.asfloat <= 0))
         then begin
           ShowMessage('O Valor Real não está preenchido! é necessário para contabilização do cliente.');
        ...
        if (tbNumerariosdata_registro.IsNull)
         then begin
           ShowMessage('A Data de registro não está preenchida! é necessária para contabilização do cliente.');
     ...
     if tbnumerarioscontabilizado_cliente.asinteger = 1
      then begin
        ...
        //incluir créditos correspondentes
        with q_exccred do
         begin
        ...
        tbnumerarioscontabilizado.asstring := 'NÃO';
        tbnumerariosvalor_contabilizado.asfloat := 0;
        tbnumerariosdetalhe.asstring := '';

        //exclui créditos correspondentes
        with q_exccred do
         begin
           params[0].asinteger := tbnumerariosreg.asinteger;
           Execsql;
     ...
     // Eduardo Souza 08/09/2011 - Controle de Autorização de Numerário
     {$IFDEF Testando}
         ...
         then begin
           if ( qryAutorizacaoSolicitacao['Status_Atual'] = 4 ) or
            ...
            then Begin
              ...
              if (Operacao = tUpdate)
               then qryUpdateSolicitacao.ParamByName('OBS_Cancelamento').Value := 'Motivo do cancelamento: Alteração dos dados na tela de numerários.'
               else qryUpdateSolicitacao.ParamByName('OBS_Cancelamento').Value := 'Motivo do cancelamento: Complemento de Numerário.';
...
begin
  ...
  if tbNumerarios.State In [dsInsert, dsEdit]
   then begin
     MessageDlg('O Numerário está em edição!', mtInformation,[mbOk], 0);
     CanClose := False;
   end
   else CanClose := True;
end;
...
begin
  ...
  if tbNumerariosPrevisao_Solicitada.AsString = 'SIM'
   then tbNumerariosPrevisao_Solicitada.AsString := 'NÃO'
  ...
  if ((Operacao = tInsert) or (Operacao = tUpdate))
   then begin
     if dbcb_contabilizado.Checked
      then begin
        Showmessage(v_usuario+', favor informar as datas/valores reais corretamente para o fechamento contábil do processo.');
        dbEdtDtReal.Enabled  := true;
        dbEdtValReal.Enabled := true;
        dbedtdtreal.SetFocus;
      end
...
procedure TF_proNumerarios.dbGrdNumerariosDrawColumnCell(Sender: TObject;
  ...
  State: TGridDrawState);
begin
  if (qrnumerariosdeb_cli.asstring = 'NÃO')
    ...
    try
      ...
      if ( qryAutorizacaoSolicitacao.IsEmpty )
       then begin
         ...
         if MessageDlg('Processo não possui solicitação de autorização de numerários. Deseja realizar a solicitação agora?', mtConfirmation,[mbYes, mbNo], 0) = mrYes
          then begin
         ...
         if ( qryAutorizacaoSolicitacao['Status_Atual'] = 1 )
          then begin
            MessageDlg( 'Já existe um pedido de solicitação de autorização de numerários, solicitado por ' + qryAutorizacaoSolicitacao['Solicitante'] + ', que ainda não foi autorizado.', mtInformation, [mbOK], 0 );
            GoOn := False;
          end
