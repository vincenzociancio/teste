...
procedure Tf_invinicial.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
action := cafree;
f_invinicial := nil;
f_mscomex.InventrioInicial1.enabled := true;

end;

procedure Tf_invinicial.b_fechaiiClick(Sender: TObject);
begin
f_invinicial.close;
end;

procedure Tf_invinicial.B_okClick(Sender: TObject);
begin
...
procedure Tf_invinicial.B_cancelaClick(Sender: TObject);
begin
     p_qprodutos.visible := false;
     me_produto.setfocus;
end;

procedure Tf_invinicial.me_numeroserieKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = chr(13) then me_quantidade.setfocus;
end;

procedure Tf_invinicial.me_quantidadeKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tf_invinicial.b_incluiClick(Sender: TObject);
begin
    ...
    if t_movinv.FindKey([trim(t_locaisimportador.asstring),trim(t_locaiscontrato.asstring),trim(t_locaislocal.asstring),me_produto.text,me_numeroserie.text,'INICIAL','.','.','.','.']) then begin
       MessageDlg('Inventário Inicial já Cadastrado para este Produto!', mtInformation,[mbOk], 0);
       end
    else begin
       t_movinv.append;
       ...
       T_movinvProcesso.asstring      := 'INICIAL';
       T_movinvDI.asstring            := '.';
       T_movinvADICAO.asstring        := '.';
       T_movinvtipo_declarao.asstring := '.';
       T_movinvtipo.asstring          := '3 ';
...
procedure Tf_invinicial.DBG_inventinicialCellClick(Column: TColumn);
begin
b_inclui.enabled := false;
...
procedure Tf_invinicial.b_excluiClick(Sender: TObject);
begin
    if not (t_movinv.FindKey([trim(t_locaisimportador.asstring),trim(t_locaiscontrato.asstring),trim(t_locaislocal.asstring),me_produto.text,me_numeroserie.text,'INICIAL','.','.','.','.'])) then begin
       MessageDlg('Inventário Inicial não Cadastrado para este Produto!', mtInformation,[mbOk], 0);
       end
    else begin
       t_movinv.delete;
...
procedure Tf_invinicial.me_produtoExit(Sender: TObject);
begin
...
procedure Tf_invinicial.me_produtoKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tf_invinicial.FormActivate(Sender: TObject);
begin
f_invinicial.left := 0;
f_invinicial.top  := 0;
...
procedure Tf_invinicial.me_dataKeyPress(Sender: TObject; var Key: Char);
begin
if key = chr(13) then me_produto.setfocus;
end;

procedure Tf_invinicial.me_quantidadeChange(Sender: TObject);
begin
b_inclui.enabled := true;
end;

procedure Tf_invinicial.T_locaisAfterScroll(DataSet: TDataSet);
begin
...
procedure Tf_invinicial.me_descricaoKeyPress(Sender: TObject;
  var Key: Char);
begin
