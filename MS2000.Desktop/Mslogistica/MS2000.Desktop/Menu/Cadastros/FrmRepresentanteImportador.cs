...
{
    public partial class FrmRepresentanteImportador : MS2000.Desktop.Componentes.MSForm01
    {
        ...
        public FrmRepresentanteImportador()
        {
            InitializeComponent();
        }
        ...
        private DataTable getRepresentante()
        {
            ...
            string query = "SELECT ID,FK_Codigo,Nome,Cargo,Identidade,CPF,Nacionalidade,Estado_civil,Exibir,Identificacao,Sexo,RG_Estrangeiro " +
                           ...
                           "WHERE FK_Codigo = @FK_Codigo  and Tipo = 2 " +
                           "ORDER BY Nome";
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@FK_Codigo", ID_Importador);
        ...
        private void FrmRepresentanteImportador_Load(object sender, EventArgs e)
        {
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                if (statusCadastro == StatusCadastro.Editar)
                {
                    ...
                    LogSistemas.AddLogSistema(((DataRowView)bindingSource.Current).Row["ID"].ToString(), FrmPrincipal.PK_UsuarioLogado, StatusCadastro.Editar, Convert.ToInt32(this.Tag), string.Format("Nome: {0} - Razao Social {1} - Tipo{2}", txtNome.Text, Razao_social, "2"));
                }
                else
                {
                    SqlConnection conn = DatabaseSqlConnection.Instance.Conexao;
                    ...
                    LogSistemas.AddLogSistema(FrmPrincipal.PK_UsuarioLogado, StatusCadastro.Novo, Convert.ToInt32(this.Tag), string.Format("Nome: {0} - Razao Social {1} - Tipo{2}", txtNome.Text, Razao_social, "2"));
                    bindingSource.DataSource = getRepresentante();
                }
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Tem certeza que deseja excluir o registro?", "Excluir", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                try
                {
                    ...
                    LogSistemas.AddLogSistema(((DataRowView)bindingSource.Current).Row["ID"].ToString(), FrmPrincipal.PK_UsuarioLogado, StatusCadastro.Excluir, Convert.ToInt32(this.Tag), string.Format("Nome: {0} - Razao Social {1} - Tipo{2}", txtNome.Text, Razao_social, "2"));
