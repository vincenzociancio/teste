...
{
    public partial class FrmContratoFiancaRepetro : MS2000.Desktop.Componentes.MSForm01
    {
        ...
        public FrmContratoFiancaRepetro()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@FK_Fiador", (string.IsNullOrEmpty(mcbbFiador.SelectedValue.ToString())) ? (object)DBNull.Value : mcbbFiador.SelectedValue);
                        cmd.Parameters.AddWithValue("@Cidade", txtCidade.Text);
                        cmd.Parameters.AddWithValue("@Repetro", true);
                        cmd.ExecuteNonQuery();
                    }
        ...
        private void dataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView.Columns[e.ColumnIndex].Name == "PDF")
            {
                ...
                string query = "select PCG.ID, P.Codigo,P.Tipo,P.Tipo_Declaracao,P.NR_DECLARACAO_IMP as DI,I.Codigo as CodigoImportador, upper(i.Razao_Social) AS Importador,i.CNPJ_CPF_COMPLETO ,i.CGC_CPF,i.Endereco,i.Numero,i.Complemento,i.Bairro,i.Cidade,i.CEP,i.UF,i.Estado , " +
                               ...
                               "C.Vigencia_inicial,C.Vigencia_final,C.Data_prorrogacao," +
                               "CASE DT_REGISTRO_DI WHEN '' THEN '' ELSE LTRIM(RTRIM(SUBSTRING([DT_REGISTRO_DI],1,2)))+'/'+LTRIM(RTRIM(SUBSTRING([DT_REGISTRO_DI],3,2)))+'/'+LTRIM(RTRIM(SUBSTRING([DT_REGISTRO_DI],5,4))) END AS [DT_REGISTRO_DI],	" +
                               "P.DT_DESEMBARACO,RR.Num_Rat as NumeroRATRepetro, (select sum(VL_ACRESCIMO) from Acrescimos_ICMS A inner join Faturas F on A.Fatura_Id = F.Fatura_Id where F.Processo = p.codigo) as ValorAFRMM, " +
                               ...
                               "F.UF as UFFiador,F.Estado as EstadoFiador ,F.Vinculacao,F.Remunerada,F.Patrimonio " +
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            try
                            {
                                ...
                                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                                {
                                    ...
                                    RV.LocalReport.ReportEmbeddedResource = "MS2000.Desktop.Menu.Processos.rptCartaGarantidor.rdlc";
                                    ...
                                    parametros.Add(new ReportParameter("Rat", reader["NumeroRATRepetro"].ToString()));
