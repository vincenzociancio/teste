...
{
    public partial class FrmTiposCustos : MS2000.Desktop.Componentes.MSForm01
    {
        ...
        public FrmTiposCustos()
        {
            InitializeComponent();
        }

        private void FrmTiposCustos_Load(object sender, EventArgs e)
        {
            getItensRateio();
            ...
            cbbRateio.DataBindings.Add(new Binding("SelectedValue", bindingSource, "Rateio"));            
        }
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Tem certeza que deseja excluir o registro ?", "Excluir", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                try
                {
                    ...
                    string query = " DELETE FROM Tipos_Custos WHERE Codigo = @Codigo ";
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    SqlConnection conn = DatabaseSqlConnection.Instance.Conexao;
                    string query = "UPDATE Tipos_Custos SET Descricao = @Descricao, Rateio = @Rateio WHERE Codigo = @Codigo ";

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@Codigo", txtCodigo.Text);
                        cmd.Parameters.AddWithValue("@Descricao", txtDescricao.Text);
                        cmd.Parameters.AddWithValue("@Rateio", (cbbRateio.SelectedValue == null ? "" : cbbRateio.SelectedValue));
                ...
                {
                    SqlConnection conn = DatabaseSqlConnection.Instance.Conexao;
                    string query = "INSERT INTO Tipos_Custos               " +
                                   " ( Codigo,  Descricao,  Rateio) VALUES " +
                                   " (@Codigo, @Descricao, @Rateio)        ";
        ...
        private DataTable getTiposCustos()
        {
            SqlConnection conn = DatabaseSqlConnection.Instance.Conexao;
            string query = "   SELECT Codigo, Descricao, Rateio " +
                           "     FROM Tipos_Custos              " +                           
                           " ORDER BY Descricao                 ";
        ...
        private Boolean CodigoJaEmUso(String Codigo)
        {
            SqlConnection conn = DatabaseSqlConnection.Instance.Conexao;
            string query = "SELECT Codigo FROM Tipos_Custos WHERE Codigo = @Codigo ";
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            MessageBox.Show("Código já em uso!", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Stop);
        ...
        private Boolean ValidaCampos()
        {
            ...
            {
                if (Util.CodigoOuDescricaoJaExistente("Tipos_Custos", txtDescricao.Text, txtCodigo.Text, statusCadastro))
                {
        ...
        {
            bindingSource.DataSource = getTiposCustos();

            dataGridView.DataSource  = bindingSource;
        }        

        private void getItensRateio()
        {
            Dictionary<string, string> itensRateio = new Dictionary<string, string>();

            itensRateio.Add("","");
            itensRateio.Add("PESO","Peso");
            itensRateio.Add("VALOR","Valor");

            cbbRateio.DisplayMember = "Value";
            cbbRateio.ValueMember   = "Key";
            cbbRateio.DataSource    = new BindingSource(itensRateio, null);
        }
